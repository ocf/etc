...
def get_cache_dir() -> Path:
    p = os.getenv("OCF_CACHE_DIR", os.getenv("XDG_CACHE_HOME", Path.home() / ".cache"))
...
def update_etc(repo_url: str, repo_path: str, out_path: Path):
    ...
    check_call(["rsync", "-rlpt", "--delete", f"{repo / repo_path}/", out_path])


def main():
    parser = argparse.ArgumentParser(description="Sync etc files from a git repo")
    parser.add_argument("out_path", type=Path, default=Path("/etc/ocf"))
    parser.add_argument("--repo-url", type=str, default="https://github.com/ocf/etc")
